...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Para el pivote (rotación con respecto al punto) primero roto y luego trasladao
		...
		model = glm::rotate(model, glm::radians(codo), glm::vec3(0.0f, 1.0, 0.0f));//Pivote
		...
		model = glm::rotate(model, glm::radians(muneca), glm::vec3(1.0f, 0.0, 0.0f));//Pivote
		modelTemp2 = modelTemp = model = glm::translate(model, glm::vec3(0.25f, 0.0f, 0.0f)); //Mover las dependencias, punto de referencia
		...
		//////Dedo 2
		model = glm::translate(modelTemp2, glm::vec3(0.25f, 0.35f, 0.0f)); //Desde el punto anteior
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		...
		//////Dedo 4 (Pulgar)
		model = glm::translate(modelTemp2, glm::vec3(0.2f, -0.2f, 0.375f));
...
void Inputs(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS && codo <= 0.0f)
		codo += 0.18f;
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && codo >= -130.0f)
		codo -= 0.18f;
	...
	if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS && dedo1 <= 10.0f)
		dedo1 += 0.18f;
	if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS && dedo1 >= -50.0f)
		dedo1 -= 0.18f;
